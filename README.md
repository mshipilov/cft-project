...
2. Запустить приложение локально и базу данных в докер контейнере

Если локальный порт 5432 занят, можно поменять его в файле .env в корне проекта
...
`poetry run uvicorn src.main:app --host 0.0.0.0 --port 8008` (при необходимости поменяйте порт)
...
## Тестирование
Для тестирования сервиса без Докер контейнера
